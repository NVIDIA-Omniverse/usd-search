...
Before asking for any `*_API_KEY` or URL token, grep
...
Ask the user once. Three options — these match the three lanes in
`docs/quickstart-user-journey.svg`:
...
- **"Local deployment"** — one-command setup. Follow the
  `/deploy-usdsearch` workflow at
  `.codex/skills/deploy-usdsearch/SKILL.md`. When it returns it will
  have set `USD_SEARCH_API_URL` for you. Then proceed to Step 2.

## Step 2 — Hand off to /search with a seed query

Follow the `/search` workflow at `.codex/skills/search/SKILL.md` with
a suggested seed query. Offer four one-click nouns plus an "Other"
escape so the "I don't know what to try" path is one click and a
specific request is exact:
...
`manifest.json`, and inspects the JPEGs via `codex exec --image`.
