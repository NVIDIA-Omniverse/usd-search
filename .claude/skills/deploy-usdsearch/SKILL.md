...
top-level compose files at the repo root. Guide the user through
configuration choices, then start docker compose in the background.
...
Use `AskUserQuestion`. Pose the question with this wording verbatim:
"Which storage backend should USD Search index?"

The four options break into two conceptual groups — **"Public Asset
Library"** and **"Custom Library"**. `AskUserQuestion` does not
support section dividers, so prefix each option's label with its
group name in square brackets so the hierarchy is visible in the
picker (e.g. `[Public Asset Library] Public S3`).
...
- The `AskUserQuestion` tool auto-appends an "Other" option ("Type
  something else") for free-form input; do not add it manually.
...
If the user picks **D (Nucleus)**, follow up asking for:
...
If the user picks A, follow up with **provider**:
...
`qwen`, `qwen_alibaba`, `mistralai`), they can pick "Other".
...
3. **AWS credentials env-var names** (`AWS_ACCESS_KEY_ID`-shaped names
   ...
   public.
4. **Is the bucket read-only?** Same wording and option pattern as the
   compose Custom S3 branch:
   - A) **Writable** — Default. Workers will upload
...
Ask each as its own `AskUserQuestion`:
