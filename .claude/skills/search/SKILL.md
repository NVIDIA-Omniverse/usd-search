...
API, and present ranked thumbnails. This replaces the older
`/search-assets` (text) and `/find-similar` (image) skills — one
endpoint, one skill, three input modes.
...
when the captions disagree AND the VLM validator is unavailable: `Read`
the saved JPEGs into the conversation in rank order. For each, judge:
- Does it match the user's words / reference image? (yes / partial / no)
- What is it? (object, colour, material)

Keep only the clear matches. Drop partials and mismatches.
...
Otherwise, end with a short pointer list (not a question). These follow-ons
ship today as both slash commands and auto-invoked skills — the user can name
them or just describe what they want:
- `/inspect-asset <asset_url>` — thumbnails, scene summary, dependencies for one hit
- `/search-in-scene <scene_url> "<spatial query>"` — radius / bbox / prim-type filters inside a scene
