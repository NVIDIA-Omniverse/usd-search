...
One `AskUserQuestion`. Three options — these match the three lanes
in `docs/quickstart-user-journey.svg`:
...
- **"Local deployment"** — one-command setup. Hand off to
  `Skill(deploy-usdsearch)`. When it returns it will have set
  `USD_SEARCH_API_URL` for you. Then proceed to Step 2.

## Step 2 — Hand off to /search with a seed query

Invoke `Skill(search)` with a suggested seed query. Offer four
one-click nouns plus an "Other" escape so the "I don't know what to
try" path is one click and a specific request is exact:
...
`manifest.json`, and `Read`s the JPEGs into chat.
