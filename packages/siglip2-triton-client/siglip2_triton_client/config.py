...
class TritonClientSettings(BaseSettings):
    ...
    triton_server_headers: Optional[Dict[str, str]] = Field(default=None, description="Metadata for Triton server")
    ...
    max_retries: int = Field(default=3, description="Max retries for transient gRPC failures")
    retry_base_delay: float = Field(default=0.1, description="Base delay in seconds for exponential backoff")
